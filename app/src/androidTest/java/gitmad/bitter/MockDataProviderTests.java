...
public class MockDataProviderTests extends InstrumentationTestCase {
    ...
    public void testGetPosts() {
        ...
        assertEquals("Posts array should contain number of posts queried", numPosts, postsArray.length);
        for (int i = 0; i < numPosts; i++) {
            assertNotNull("no index in array returned should be null", postsArray[i]);
        }
    }
    ...
    public void testAddPost() {
        ...
        assertEquals("Should be able to get added post from postProvider#getPosts()",
    ...
    public void testDownvote() {
        ...
        assertEquals("Returned post should have one less(?) downvote.",
                firstPostInFeed.getDownvotes() - 1, downvotedPost.getDownvotes());
        assertEquals("Stored post should have one less(?) downvote.",
                firstPostInFeed.getDownvotes() - 1, postProvider.getPost(postId).getDownvotes());
    }
    ...
    public void testDeletePost() {
        ...
        assertTrue("Feed should have first post removed", Arrays.equals(
                postList.toArray(new Post[postList.size()]), postProvider.getPosts(numPostsToRetrieve - 2)));
    }
    ...
    public void testGetCommentsOnPost() {
        ...
        for (Comment comment : comments) {
            assertEquals("Comments should all have post Id requested", postId, comment.getPostId());
        }
    }
