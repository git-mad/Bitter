...
public class MockPostProvider implements PostProvider {
    ...
    public Post addPostSync(String postText) {
        logMessageIfEnabled(String.format("MockPostProvider#addPost(%s)",
                postText));

        String nextId = Integer.toString(getNextId());
        Post newPost = createPostWithText(postText, nextId,
                getMockLoggedInUserId());
    ...
    public Post deletePost(String postId) {
        logMessageIfEnabled(String.format("MockPostProvider#deletePost(%s)",
                postId));
    ...
    public Post downvotePost(String postId) {
        logMessageIfEnabled(String.format("MockPostProvider#downvotePost(%s)" +
                "", postId));
    ...
    public Post[] getPostsByUser(String userId) {
        logMessageIfEnabled(String.format("MockPostProvider#getPostsByUser" +
                "(%s)", userId));
    ...
    private Post createPostWithText(String text, String postId, String
            authorId) {
        long postCreatedTimestamp = new Date().getTime();
        final int zeroDownvotes = 0;
        return new Post(postId, text, postCreatedTimestamp, zeroDownvotes,
                authorId, "sports");
    }
    ...
    private Post createRandomPostWithText(String text, String postId) {
        ...
        return new Post(postId, text, postCreatedTimestamp, 0,
                randomAuthorId, "sports");
    }
    ...
    private void initializeMockPosts() {
        ...
        /*
            create new posts with ids 0...postsText.length, and use these
            id's as keys
            in the Map.
         */
        for (int i = 0; i < postsText.length; i++) {
            posts.put(Integer.toString(i), createRandomPostWithText
                    (postsText[i], Integer.toString(i)));
        }
    }
